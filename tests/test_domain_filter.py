...
def test_multi_domain_unit_matches():
    """A unit with multiple domains should match if any domain matches the query."""
    units = [
        _make_unit("A", ["causal_inference"], [1.0, 0.0, 0.0]),
        _make_unit("B", ["survey_sampling"], [0.9, 0.1, 0.0]),
        _make_unit("C", ["causal_inference", "bayesian"], [0.8, 0.2, 0.0]),
    ]
    query_emb = [1.0, 0.0, 0.0]
    scored = _score_methods(query_emb, units, domain="causal_inference")
    titles = [u["title"] for _, u in scored]
...
def test_legacy_string_domain_backward_compat():
    """A unit with a plain string domain (legacy) should still match."""
    units = [
        _make_unit("A", "causal_inference", [1.0, 0.0, 0.0]),
        _make_unit("B", "bayesian", [0.9, 0.1, 0.0]),
    ]
